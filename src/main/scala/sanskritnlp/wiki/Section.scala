...
class Section {
  ...
  def deleteSection(sectionPath: String): Boolean = {
    val sectionList = sectionPath.split("/src/main/java").filter(_ != "")
    ...
    return headSectionList.map(headSection => {
      if (sectionList.length > 1) {
        return headSection.deleteSection(sectionList.tail.mkString("/src/main/java"))
      } else {
  ...
  def getOrCreateSection(sectionPath: String, defaultSection: Section = new Section): Section = {
    ...
    if (sectionList.tail.length > 0) {
      return headSection.getOrCreateSection(sectionList.tail.mkString("/src/main/java"))
    } else {
      return headSection
    }
  }
