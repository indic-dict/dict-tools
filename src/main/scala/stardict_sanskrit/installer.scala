...
case class DictIndex(indexUrl: String, downloadPathPrefix: String, var downloadPath: String = "", var dictTarUrls: List[String] = List()) {
  downloadPath = Paths.get(downloadPathPrefix, indexUrl.trim.replace("https://raw.githubusercontent.com/", "").replace("master/", "").replace("gh-pages/", "").replace("tars/tars.MD", "")).toString
